...
export const Route = createFileRoute("/admin/finance")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "VS Store finance workspace: P&L, refunds, chargebacks and reconciliation." },
...
interface FinanceResponse {
  ...
  refundRows: Array<{ id: string; order: string; date: string; amount: number; reason: string | null }>;
  disputes: Array<{ id: string; order: string; status: string | null; initiatedAs: string | null; amount: number }>;
  recentOrders: Array<{
    id: string;
    name: string;
    email: string | null;
  ...
  reconciliation: { shopifyOrders: number; mirroredOrders: number; lastSyncedAt: string; inSync: boolean };
}
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {RANGES.map((r) => (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-sm font-medium ${
                days === r.days ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card"
              }`}
            >
              {r.label}
            </button>
          ...
          <Link to="/admin/orders" className="rounded-full border border-border bg-card px-3 py-1.5 text-sm font-medium">
            Orders
          </Link>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <Stat label="Gross sales" value={formatMoney(t['grossSales'] ?? 0, c)} hint={`${t['orders'] ?? 0} orders`} />
            <Stat label="Net sales" value={formatMoney(t['netSales'] ?? 0, c)} tone="good" hint={`AOV ${formatMoney(t['averageOrderValue'] ?? 0, c)}`} />
            <Stat label="Refunds" value={formatMoney(t['refunds'] ?? 0, c)} tone="bad" hint={`${(t['refundRate'] ?? 0).toFixed(1)}% of gross`} />
            <Stat label="Chargebacks" value={formatMoney(t['chargebackAmount'] ?? 0, c)} tone="bad" hint={`${t['chargebacks'] ?? 0} disputes`} />
          </div>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={String(label)} className="border-b border-border/60 last:border-0">
                      ...
                      <td className={`py-2 text-right font-medium ${Number(value) < 0 ? "text-signal" : ""}`}>
                  ...
                  <tr>
                    ...
                    <td className="py-2 text-right font-medium">{(t['margin'] ?? 0).toFixed(1)}%</td>
                  </tr>
                </tbody>
              </table>
            </div>
                    ...
                    <tbody>
                      {data.refundRows.map((r) => (
                        <tr key={r.id} className="border-b border-border/60 last:border-0">
                          ...
                          <td className="py-2 text-right text-signal">{formatMoney(r.amount, c)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="mt-6 vs-card rounded-2xl border border-border bg-card p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-4 text-sm">
              ...
              <div>
                ...
                <p className={`font-semibold ${data.reconciliation.inSync ? "text-primary" : "text-signal"}`}>
                  {data.reconciliation.inSync ? "In sync" : "Drift detected"}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground">Last synced</p>
                <p className="font-semibold">{new Date(data.reconciliation.lastSyncedAt).toLocaleString()}</p>
              </div>
            </div>
            <button
              onClick={async () => {
